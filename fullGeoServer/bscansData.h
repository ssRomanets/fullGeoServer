...
enum class BscanFileFormat {Trz, Csv, Hdf5};

struct st_bscanInitialData {
    const char* header; // bscans  (//3 вводных байта)
...
struct st_bscanInfoData {
    std::vector<st_bscanInitialData>                            vectorBscanData;
    std::vector<double>                                         vectorMinLog10BscanData;
    std::vector<double>                                         vectorMaxLog10BscanData;
    std::vector<std::vector<QPair<int, int> >>                  vectorsBscanLog10PairXY;
    std::vector<std::map<int, std::vector<QPair<int, int> > >>  vectorMapAutoLog10BscanPairXY;
    double                                                      contrastLog10BscanKoeff{0.0};
    int quantImpulsesOfPacket {0};
};

struct st_bscansWorkData {
    int bscansSurfMinWidth {0};
    int bscansSurfWidth    {0};
    int bscansSurfHeight   {0};
    ...
    std::vector<std::tuple<std::string, int> >  vectorBscansData;
    std::vector<std::vector<QVector3D>>     vectorBscansInRelief;
    std::vector<std::vector<double> >       vectorBscansFnRelief;
};
